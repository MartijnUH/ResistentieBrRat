...
parameters {
  array[K] vector[R] eps;                     // Site random effect
  array[K] real bs0;                          // Slope for time effect per mutation
  array[K] vector[R] bs;                      // Site random slopes
...
transformed parameters {
  ...
  for (k in 1:K) {
    ...
    conv_re[k] = sqrt(1 - rho_re[k]) * eps[k] + sqrt(rho_re[k] / scaling_factor) * U[k];
    
    if (k == 1 || k == 2) {
      Bs[k] = HSGP_2D(R, phi, rho_b[k], 1, eta_b[k], xy_lim, xy_design, n_bf);
      conv_rs[k] = sqrt(1 - rho_rs[k]) * bs[k] + sqrt(rho_rs[k] / scaling_factor) * Bs[k];
    } else {
      conv_rs[k] = bs[k];
    }
