...
data {
  ...
  int<lower=1> Rs;                          // Number of grid cells sampled
  array[Rs] int<lower=1> rs;                // Id's of sampled grid cells
  array[R-Rs] int<lower=1> rns;             // Id's of unsampled grid cells
...
parameters {
  array[K] vector[Rs] eps;                    // Site random effect
  array[K] real bs0;                          // Slope for time effect per mutation
  array[K] vector[Rs] bs;                     // Site random slopes
  ...
  array[K] real<lower=0> sigma_re;            // overall standard deviation - re
  array[K] real<lower=0> sigma_rs;            // overall standard deviation - rs
  array[K] real<lower=0, upper=1> rho_re;     // proportion unstructured vs. spatially structured variance - re
  array[K] real<lower=0, upper=1> rho_rs;     // proportion unstructured vs. spatially structured variance - rs
}
...
transformed parameters {
  ...
  array[K] vector[R] conv_re;               // Combined random effects
  array[K] vector[R] conv_rs;               // Combined random slopes
  array[R, T] simplex[K] prop;              // Proportions of each mutation (softmax)

  for (k in 1:K) {
    U[k] = HSGP_2D(R, phi, rho_u[k], 1, eta_u[k], xy_lim, xy_design, n_bf);
    conv_re[k][rs] = sqrt(1 - rho_re[k]) * eps[k] + sqrt(rho_re[k] / scaling_factor) * U[k][rs];
    conv_re[k][rns] = sqrt(1 / scaling_factor) * U[k][rns];
    
    if (k == 1 || k == 2) {
      Bs[k] = HSGP_2D(R, phi, rho_b[k], 1, eta_b[k], xy_lim, xy_design, n_bf);
      conv_rs[k][rs] = sqrt(1 - rho_rs[k]) * bs[k] + sqrt(rho_rs[k] / scaling_factor) * Bs[k][rs];
      conv_rs[k][rns] = sqrt(1 / scaling_factor) * Bs[k][rns];
    } else {
      conv_rs[k][rs] = bs[k];
      conv_rs[k][rns] = rep_vector(0, R-Rs);
    }
    B[k] = rep_vector(bs0[k], R) + (conv_rs[k] * sigma_rs[k]);
  }
  ...
  for (i in 1:R) {
    for (t in 1:T) {
      vector[K] logits;
      for (k in 1:K)
        logits[k] = logit_mu[k] + (B[k][i] * year[t]) + (conv_re[k][i] * sigma_re[k]);
...
model {
  ...
  to_vector(sigma_re) ~ student_t(3, 0, 5);
  to_vector(sigma_rs) ~ student_t(3, 0, 5);
  to_vector(rho_re) ~ beta(5, 2);
  to_vector(rho_rs) ~ beta(5, 2);

  for (k in 1:K) {
    eps[k] ~ normal(0, 1);
    bs[k] ~ normal(0, 1);
    sum(eps[k]) ~ normal(0, 0.001 * Rs);
    sum(bs[k]) ~ normal(0, 0.001 * Rs);
