...
parameters {
  ...
  array[K] real<lower=0> sigma;               // overall standard deviation
  array[K] real<lower=0, upper=1> rho;        // proportion unstructured vs. spatially structured variance
}
...
transformed parameters {
  ...
  for (k in 1:K) {
    ...
    conv_re[k][rns] = U[k][rns];
    conv_re[k][rs] = sqrt(1 - rho[k]) * eps[k] + sqrt(rho[k] / scaling_factor) * U[k][rs];
  }
  ...
  for (i in 1:R) {
    for (t in 1:T) {
      vector[K] logits;
      for (k in 1:K)
        logits[k] = logit_mu[k] + (B[k][i] * year[t]) + (conv_re[k][i] * sigma[k]);
...
model {
  ...
  to_vector(sigma) ~ student_t(3, 0, 5);
  to_vector(rho) ~ beta(5, 2);
